...
function ServersPage() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["servers"] }); setOpen(false); toast.success("Server added!"); },
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="rounded-3xl">
            ...
            <form className="flex flex-col gap-4 mt-2" onSubmit={(e) => { e.preventDefault(); addMutation.mutate(form); }}>
              <div className="grid gap-2">
                <Label>Label</Label>
                <Input placeholder="e.g. NY-01" value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} required className="rounded-xl" />
              </div>
              <div className="grid gap-2">
                <Label>Hostname</Label>
                <Input placeholder="mail.example.com" value={form.hostname} onChange={e => setForm(f => ({ ...f, hostname: e.target.value }))} required className="rounded-xl" />
              </div>
              <div className="grid gap-2">
                <Label>IP Address</Label>
                <Input placeholder="1.2.3.4" value={form.ipAddress} onChange={e => setForm(f => ({ ...f, ipAddress: e.target.value }))} required className="rounded-xl" />
              </div>
              <div className="grid gap-2">
                <Label>SSH User</Label>
                <Input placeholder="root" value={form.sshUser} onChange={e => setForm(f => ({ ...f, sshUser: e.target.value }))} className="rounded-xl" />
              </div>
              <Button type="submit" disabled={addMutation.isPending} className="bg-[#4DB584] hover:bg-[#3da070] rounded-xl mt-2">
                {addMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Add Server"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {serverList.map((s: any) => (
            <div key={s.id} className="rounded-3xl bg-white p-6 ring-1 ring-black/5 shadow-sm flex flex-col gap-3">
